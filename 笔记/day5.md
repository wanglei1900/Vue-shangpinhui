...
3）对于typeNav请求次数也进行优化（放入根组件中，只会执行一次）
...
this.$nextTick:可以保证页面中的结构一定是由的，经常和很多插件一起使用，【都需要DOM存在了】
...
<!-- !直接在子组件上用v-for遍历，且向子组件props传值时，props传值注意传的必须是遍历项（理解：每个遍历的子组件传入对应的遍历项） -->

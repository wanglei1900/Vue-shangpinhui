3)复习一下（前三天的难点复习）
<!-- !商品分类的三级列表由静态变为动态形式【获取服务器数据：解决代理跨域问题webpack-devServer-proxy】 -->
<!-- !函数的防抖与节流【面试的频率很高 lodash，debounce(防抖函数)， throttle（节流函数）】 -->
<!-- !路由的跳转与传参：声明式导航与编程式导航,编程式导航解决这个问题：采用自定义属性 -->
<!-- !前面基础课程当中v-for很少使用index，以后在写项目的时候，index索引值切记加上 -->
<!-- !vuex可以模块式开发,vuex经常用的套路是state、mutations、actions、getters、modules -->

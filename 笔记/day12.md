...
登录成功，服务器会下放token，前台持久化存储token（项目里接口）
...
### 路由独享守卫：4.1用户已经登录了，不应该在访问login？【通过什么条件能判断用户登录、未登录】针对某一个路由的守卫
举例子：紫禁城【皇帝、太后、妃子】，是相应的【皇帝、太子、妃子】路上守卫

### 组件内守卫：也是负责某一个路由守卫
举例子：已经到了皇帝物资外面，还是有守卫

            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a>综合 <span v-show="isOne" class="iconfont" :class=" {'icon-shangjiantou':isAsc,'icon-xiajiantou':isDesc}"></span> </a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a>价格 <span v-show="isTwo" class="iconfont" :class=" {'icon-shangjiantou':isAsc,'icon-xiajiantou':isDesc}"></span> </a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  beforeMount() {
    ...
    this.searchParams.keyword = this.$route.params.searchWord */
  ...
  computed: {
    // mapGetters里面的写法：传递的数组。开启命名空间区分模块
    ...mapGetters("searchStore", ["goodsList", "trademarkList", "attrsList"]),
    isOne(){
    ...
    // 获取search模块接口里的total参数,可以和上面那个getters合并，但是这里学习阶段为了理解不和合并
    ...mapGetters('searchStore',['total'])
  },
  ...
  methods: {
    ...
    getData() {
      // 测试接口返回的数据格式
      this.$store.dispatch("searchStore/getSearchList", this.searchParams);
    },
    // 删除分类的名字
    removeCategoryName(){
      // 把带给服务器的参数置空了，还需要向服务器发送请求
      // 带给服务器多数说明可有可无的，如果属性值为空的字符串还是会把相应的字段带给服务器
      ...
      // 严谨：本意是删除query，如果路径当中出现params不应该删除，并且路由跳转的时候应该带着
    ...
    // 删除关键字
    removeKeyword(){
      // 给服务器代的参数searchParams的keyword置空
  ...
  watch: {
    ...
    $route:{
      ...
      handler(newValue,oldValue){
        ...
        // 再次发起ajax请求
        this.getData()
        // 每次请求完毕，应该把相应的1,2,3集级分来的id清空，让他接收下一次的响应1,2,3id
        // 为什么不需要置空搜索关键字keyword，因为每一次路由发生变化的时候，都会给他赋予新的数据
        this.searchParams.category1Id=undefined
        this.searchParams.category2Id=undefined
        this.searchParams.category3Id=undefined
      }
